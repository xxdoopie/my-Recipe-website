...
function Home({sendQueryFromNav, sendUpdatedPage}) {
    ...
    return (
        ...
        <div className="suggestionSection flex flex-col">
          <div className="flex justify-between">
            ...
            <h4 className="text-gray-500 text-sm cursor-pointer"
            onClick={() => handleSearch('salads')}
            >See all</h4>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full">
            ...
            <div className="flex flex-col gap-10">
              {[
                ...
                {
                  title: "Greek Salad",
                  desc: "Mediterranean Healthy Choice",
                  img: "/assets/caesar salad.png",
                  rating: "4.2"
                }
              ].map((item, index) => (
                <div 
                  key={index} 
                  className="benefitCard flex items-center gap-3 p-3 bg-white rounded-xl shadow-md w-full cursor-pointer hover:bg-gray-50 transition-colors"
                  onClick={() => handleSearch(item.title)}
                >
            ...
            <div className="flex flex-col gap-6 col-span-1 md:col-span-1 w-full">
              ...
              {/* Weekly Pick */}
              <div
                className="weeklyPick relative text-white flex flex-col w-full h-[300px] px-4 py-10 sm:p-10 justify-between bg-cover bg- bg-center bg-no-repeat rounded-xl cursor-pointer"
                style={{
                  backgroundImage: "url('/assets/WeeklyPicImage.png')",
                }}
                onClick={() => handleSearch("Pumpkin Cream Soup")}
              >
                <div className="flex justify-between w-full">
                  ...
                  />
                  <button 
                    className="px-4 py-1 h-[40px] bg-mainTheme rounded-xl text-white font-bold text-sm cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleSearch("Pumpkin Cream Soup");
                    }}
                  >
                    Start cook
                  </button>
